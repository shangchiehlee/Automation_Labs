...
SUPPORTED_INPUT_EXTENSIONS: Tuple[str, ...] = ('.xlsx', '.xlsm', '.csv')
...
def find_input_file(search_dir: Path, partial_name: str) -> Optional[Path]:
    """
    Locates the first supported input file matching the partial name pattern.

    Supports .xlsx, .xlsm and .csv. If multiple matches exist, prefers Excel files.
    Ignores temporary Excel lock files that begin with "~$".
    """
    if not search_dir.exists():
        logging.error(f"Input directory does not exist: {search_dir}")
        return None
    ...
    if not matches:
        ...
        logging.error(
            "No file found matching '%s' in %s. Supported extensions: %s. Visible files: %s",
            partial_name,
            search_dir,
    ...
    preference_order = {'.xlsx': 0, '.xlsm': 1, '.csv': 2}
    matches.sort(key=lambda p: (preference_order.get(p.suffix.lower(), 99), p.name.lower()))

    if len(matches) > 1:
        logging.info(
            "Multiple files matched '%s'. Selected '%s'. Candidates: %s",
            partial_name,
            matches[0].name,
...
def _csv_chunk_generator(file_path: Path, chunk_size: int) -> Iterator[pd.DataFrame]:
    """
    Generator function yielding DataFrame chunks from a CSV file.
    Uses pandas chunked reading to maintain bounded memory usage.
    """
    for chunk in pd.read_csv(file_path, chunksize=chunk_size, low_memory=False):
        yield chunk


def _tabular_chunk_generator(file_path: Path, chunk_size: int) -> Iterator[pd.DataFrame]:
    """
    Dispatches to the appropriate chunk generator based on file extension.
    Supports .csv, .xlsx, and .xlsm.
    """
    suffix = file_path.suffix.lower()
    if suffix == '.csv':
        yield from _csv_chunk_generator(file_path, chunk_size)
    elif suffix in {'.xlsx', '.xlsm'}:
        yield from _excel_chunk_generator(file_path, chunk_size)
    else:
...
def build_pii_map(reference_path: Path) -> Dict[str, str]:
    ...
    generator = _tabular_chunk_generator(reference_path, chunk_size=10000)
...
def process_stream(input_path: Path, output_path: Path, engine: AnonymizerEngine, 
    ...
    """
    Streams input tabular data (Excel or CSV), applies anonymization, and appends results to CSV.
    """
    ...
    try:
        reader = _tabular_chunk_generator(input_path, chunk_size)
...
def main() -> None:
    ...
    ref_file = find_input_file(inputs_dir, config['settings']['input_pattern_reference'])
    data_file = find_input_file(inputs_dir, config['settings']['input_pattern_transaction'])
