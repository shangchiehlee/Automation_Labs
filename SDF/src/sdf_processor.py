...
class SQLiteBuffer:
    ...
    def _excel_chunk_generator(self, file_path: Path, chunk_size: int) -> Iterator[pd.DataFrame]:
        """
        Yields DataFrame chunks from an Excel file using OpenPyXL in read_only mode.
        """
    ...
    def ingest_reference_stream(self, file_path: Path, chunk_size: int) -> None:
        ...
        total_rows = 0
        for chunk in self._excel_chunk_generator(file_path, chunk_size):
    ...
    def ingest_transaction_stream(self, file_path: Path, chunk_size: int, dry_run_limit: int = 0) -> None:
        ...
        for batch_id, chunk in enumerate(self._excel_chunk_generator(file_path, chunk_size)):
...
def main() -> None:
    ...
    try:
        ...
        ref_file = next(inputs_dir.glob(f"*{ref_pattern}*.xlsx"))
        tx_file = next(inputs_dir.glob(f"*{tx_pattern}*.xlsx"))
    except StopIteration:
        logging.critical("Missing Input Files in 'inputs' folder.")
        sys.exit(1)
        ...
        with tempfile.NamedTemporaryFile(suffix='.db', delete=True) as tmp_db:
            ...
            with SQLiteBuffer(db_path) as buffer:
                try:
                    ...
                    buffer.ingest_transaction_stream(tx_file, config['performance_settings']['chunk_size'], config['performance_settings']['dry_run_limit'])
